class Boid {
    constructor (x, y) {
        ...
        this.isLeader;
        //Randomly set leaders
        if (random(100) < leaderChance) {
            this.isLeader = true;
        }
    }
    ...
    update () {
        ...
        push();
            fill(255);
            if (this.isLeader) { fill(0, 255, 0); }
            translate(this.pos.x, this.pos.y);
            rotate(this.vel.heading() + PI/2);
            triangle(5*boidSize, 10*boidSize, 0, -10*boidSize, -5*boidSize, 10*boidSize);
        pop();
    }
    ...
    alignment (neighbors) {
        ...
        for(let i = 0; i < neighbors.length; i ++) {
            let other = neighbors[i];
            //Only care if the other is a leader
            if (other.isLeader || !useLeaders) {
